    ...
    return (
        <>
            ...
            <Col>
                ...
                <Button onClick={() => dropdownMagnetToggle()} className={"mt-3 d-block"}>Показать magnet-ссылки</Button>
                <Modal size={"lg"} isOpen={isDropdownMagnetOpen} toggle={dropdownMagnetToggle}>
                    <ModalBody>
                        {
                            arr.map((item, idx) => {
                                return <Button outline color="primary" size="lg" block key={idx} href={item[1]}>{item[0].split(")")[1]}</Button>
                            })
                        }
                    </ModalBody>
                </Modal>
                ...
                <Modal size={"lg"} isOpen={isYoutubePlayerOpen} toggle={youtubePlayerToggle}>
                    <ModalBody>
                        <iframe title="Trailer" className={"mt-3"} width={"100%"} height={"500"} src={`https://www.youtube.com/embed/${film.youtube_id}`}
                            frameBorder="0"
